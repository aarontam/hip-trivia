 ...
 * @yields {[type]} [description]
 */
Trivia.prototype.api = function* (endpoint, params) {
 ...
 */
Trivia.prototype.scrubAnswer = function (answer) {
 ...
 */
Trivia.prototype.checkBySorting = function (guess) {
	var correct = true,
		tenant = this.tenants.getTenant(this.tenant.id),
...
Trivia.prototype.checkAnswer = function* (guess) {
	...
	if (tenant.getState('mode') == modes.ACTIVE) {
		...
		if (guess) {
			scrubbedGuess = this.scrubAnswer(guess);
		...
		if (correct) {
			...
			yield this.sendMessage(`${this.sender.name} is correct! ${this.generateScore(record)}<br />
			...
			yield this.sendMessage(`${this.sender.name}: ${fails[feedbackIdx]} ${this.generateScore(record)}`, {
				color: 'red',
				message_format: 'text'
			});
		}
...
Trivia.prototype.showWinnings = function* () {
	...
	for (idx = 0; idx < winnings.length; idx++) {
		...
		msg += `${player.name} ${this.generateScore(player)}<br />`;
	}
 ...
 */
Trivia.prototype.generateScore = function (record) {
