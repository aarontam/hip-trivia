...
function splitWords (s){
	// Adapted from http://stackoverflow.com/a/18679657/1569595
...
var generateHints = function (answer) {
	...
	msg = `The answer contains ${count} word${count > 1 ? 's' : ''}.`;
	hints.push(msg);
	...
	for (idx = 0; idx < count; idx++) {
		...
		level3.push(firstLetter +
			(word.length > 1 ? word.substring(1, word.length - 1).replace(reNonWhitespace, hintReplacement) : '') +
			(word.length > 2 ? lastLetter : ''));
